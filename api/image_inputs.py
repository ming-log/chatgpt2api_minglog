...
IMAGE_QUALITY_VALUES = {"auto", "low", "medium", "high"}
...
def _parse_quality(value: object) -> str | None:
    """解析图片质量：兼容 OpenAI Images API 的 quality 参数。"""
    quality = _clean(value)
    if not quality:
        return None
    if quality not in IMAGE_QUALITY_VALUES:
        raise HTTPException(status_code=400, detail={"error": "quality must be one of auto, low, medium, high"})
    return quality
...
def _payload_from_fields(fields: dict[str, Any]) -> dict[str, Any]:
    ...
    payload = {
        ...
        "account_type": _clean(fields.get("account_type"), "free"),
...
async def parse_image_edit_request(request: Request) -> tuple[dict[str, Any], list[ImageSource]]:
    ...
    fields: dict[str, Any] = {}
    for key in ("client_task_id", "prompt", "model", "account_type", "n", "size", "quality", "response_format", "stream"):
        value = form.get(key)
        if isinstance(value, str):
            fields[key] = value
    sources: list[ImageSource] = []
