...
from services.account_service import IMAGE_ACCOUNT_TYPE_ORDER, account_service, normalize_image_account_category
...
def _image_account_type(value: Any) -> str:
    return normalize_image_account_category(value)


def _image_available_quota(items: list[dict[str, Any]]) -> str:
    available = [item for item in items if str(item.get("status") or "") == "正常"]
    if any(str(item.get("type") or "").strip().lower().replace("-", "_") in {"pro", "prolite"} for item in available):
        return "∞"
    if any(bool(item.get("image_quota_unknown")) for item in available):
        return "未知"
    return str(sum(max(0, int(item.get("quota") or 0)) for item in available))
...
def create_router() -> APIRouter:
    ...
    @router.get("/api/accounts/image-types")
