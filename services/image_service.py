...
def get_image_response(relative_path: str) -> FileResponse | Response:
    if image_storage_service.has_local(relative_path):
        return FileResponse(_safe_image_path(relative_path), headers=IMAGE_CACHE_HEADERS)
    return Response(content=image_storage_service.get_bytes(relative_path), media_type="image/png", headers=IMAGE_CACHE_HEADERS)
...
def get_thumbnail_response(relative_path: str) -> FileResponse:
    return FileResponse(ensure_thumbnail(relative_path), headers=IMAGE_CACHE_HEADERS)
