...
IMAGE_ACCOUNT_TYPE_ORDER = ("free", "paid")
PAID_PLAN_TYPES = {"paid", "plus", "team", "business", "enterprise", "edu", "pro", "prolite"}


def _normalize_plan_type(value: Any) -> str:
    plan_type = _clean_string(value).lower()
    if plan_type == "codex":
        return ""
    compact = re.sub(r"[-\s/]+", "_", plan_type)
    if compact in {"team_plus_pro", "plus_pro", "team_plus", "paid"}:
        return "paid"
    for candidate in ("paid", "prolite", "enterprise", "business", "team", "plus", "pro", "edu", "free"):
        if compact == candidate or compact.endswith(f"_{candidate}") or candidate in compact.split("_"):
            return candidate
    return plan_type


def _is_paid_plan_type(value: Any) -> bool:
    plan_type = _normalize_plan_type(value)
    return plan_type in PAID_PLAN_TYPES


def normalize_image_account_category(value: Any, default: str = "") -> str:
    plan_type = _normalize_plan_type(value)
    if plan_type == "free":
        return "free"
    if _is_paid_plan_type(plan_type):
        return "paid"
    return default


def _normalize_requested_account_type(value: Any) -> str:
    return normalize_image_account_category(value) or _normalize_plan_type(value) or "free"
...
class AccountService:
    ...
    def _normalize_account(self, item: dict) -> dict | None:
        ...
        account_type = _normalize_plan_type(normalized.get("type"))
        plan_type = _normalize_plan_type(normalized.get("plan_type"))
        if (not account_type or account_type == "free") and _is_paid_plan_type(plan_type):
            account_type = plan_type
        normalized["type"] = account_type or plan_type or "free"
    ...
    def _list_ready_candidate_tokens(
        ...
        return [
            ...
            if self._is_image_account_available(item)
               and self._account_matches_type(item, account_type)
    ...
    def _list_available_candidate_tokens(
        ...
        return [
            token
            for token in self._list_ready_candidate_tokens(excluded_tokens, account_type)
    ...
    def _acquire_next_candidate_token(
        ...
        with self._image_slot_condition:
            while True:
                if not self._list_ready_candidate_tokens(excluded_tokens, requested):
                    raise RuntimeError(f"no available image quota for account type {requested}")
                tokens = self._list_available_candidate_tokens(excluded_tokens, requested)
                if tokens:
    ...
    def get_available_access_token(self, account_type: str | None = "free") -> str:
        requested = _normalize_requested_account_type(account_type)
        attempted_tokens: set[str] = set()
        while True:
            access_token = self._acquire_next_candidate_token(
                excluded_tokens=attempted_tokens,
                account_type=requested,
            )
            ...
            try:
                ...
                continue
            if self._is_image_account_available(account or {}) and self._account_matches_type(account or {}, requested):
                return access_token
            self.release_image_slot(access_token)
    ...
    def update_account(self, access_token: str, updates: dict, *, preserve_plan_hint: bool = False) -> dict | None:
        if not access_token:
            return None
        with self._lock:
            current = self._accounts.get(access_token)
            ...
            if preserve_plan_hint and _normalize_plan_type(next_updates.get("type")) == "free":
                plan_hint = _normalize_plan_type(current.get("plan_type"))
                if _is_paid_plan_type(plan_hint):
                    next_updates["type"] = plan_hint
            elif "type" in next_updates and not _is_paid_plan_type(next_updates.get("type")):
                next_updates["plan_type"] = ""
            account = self._normalize_account({**current, **next_updates, "access_token": access_token})
            if account is None:
                return None
    ...
    def fetch_remote_info(self, access_token: str, event: str = "fetch_remote_info") -> dict[str, Any] | None:
        ...
        try:
            ...
            raise
        return self.update_account(access_token, result, preserve_plan_hint=True)
