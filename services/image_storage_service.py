...
class WebDAVClient:
    ...
    def ensure_dirs(self, rel: str, *, timeout: float = 30.0) -> None:
    ...
    def put(self, rel: str, payload: bytes, content_type: str = "image/png", *, timeout: float = 30.0) -> str:
        self.ensure_dirs(rel, timeout=timeout)
        url = self.remote_url(rel)
        self._request("PUT", url, data=payload, headers={"Content-Type": content_type}, timeout=timeout)
        return url
    ...
    def delete(self, rel: str, *, timeout: float = 30.0) -> bool:
        response = self.session.request("DELETE", self.remote_url(rel), timeout=timeout, **self._auth_kwargs())
    ...
    def test(self) -> dict[str, object]:
        ...
        try:
            self.put(test_rel, b"chatgpt2api webdav test\n", content_type="text/plain", timeout=10.0)
            self.delete(test_rel, timeout=10.0)
