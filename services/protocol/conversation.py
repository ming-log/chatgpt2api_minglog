...
def image_stream_error_message(message: str) -> str:
    ...
    if (
        is_transient_upstream_connection_error(text)
        or "curl: (35)" in lower
...
def build_image_prompt(prompt: str, size: str | None) -> str:
    if not size:
        return prompt
    dimension_match = re.fullmatch(r"\s*(\d+)\s*x\s*(\d+)\s*", str(size), flags=re.IGNORECASE)
    if dimension_match:
        width, height = dimension_match.groups()
        return f"{prompt.strip()}\n\n输出图片尺寸为 {width}x{height}，尽量严格保持这个宽高比和构图范围。"
...
def stream_image_outputs(
    ...
    try:
        ...
        if not conversation_id or not is_transient_upstream_connection_error(exc):
            raise
        logger.warning({
            "event": "image_stream_interrupted_recover",
...
def stream_image_outputs_with_pool(request: ConversationRequest) -> Iterator[ImageOutput]:
    ...
    for index in range(1, request.n + 1):
        while True:
            try:
                token = account_service.get_available_access_token(request.account_type)
