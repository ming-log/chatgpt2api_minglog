...
class ResponsesTests(unittest.TestCase):
    ...
    def test_text_response_with_image_tool_still_uses_text_branch(self):
        """非图片模型即使携带 image_generation 工具也应走文本分支。"""
        response = requests.post(
            f"{BASE_URL}/v1/responses",
            headers={"Authorization": f"Bearer {AUTH_KEY}"},
            json={
        ...
        self.assertEqual(str(output[0].get("type") or ""), "message")
