...
import type { ImageAccountType, ImageModel } from "@/lib/api";
import {
...
export type StoredImageGenerationConfig = {
  model: ImageModel;
...
export type StoredReferenceImage = {
  ...
  type: string;
  dataUrl: string;
  generationConfig?: StoredImageGenerationConfig;
};
...
function normalizeStoredImage(image: StoredImage): StoredImage {
  const legacyStartedAt = (image as StoredImage & { createdAt?: unknown }).createdAt;
  const normalized = {
    ...image,
    taskId: typeof image.taskId === "string" && image.taskId ? image.taskId : undefined,
    startedAt:
      typeof image.startedAt === "string" && image.startedAt
        ? image.startedAt
        : typeof legacyStartedAt === "string" && legacyStartedAt
          ? legacyStartedAt
          : undefined,
...
function normalizeReferenceImage(image: StoredReferenceImage): StoredReferenceImage {
  const generationConfig = normalizeStoredGenerationConfig(
    (image as StoredReferenceImage & { generationConfig?: unknown }).generationConfig,
  );
  return {
...
function normalizeStoredAspectRatio(value: unknown, legacySize: unknown): ImageAspectRatio {
  return normalizeImageAspectRatio(typeof value === "string" ? value : legacySize);
}

function normalizeStoredResolution(value: unknown): ImageResolution {
  return normalizeImageResolution(value);
}

function normalizeStoredQuality(value: unknown): ImageQuality {
  return normalizeImageQuality(value);
}

function normalizeStoredAccountType(value: unknown): ImageAccountType {
...
function normalizeStoredModel(_value: unknown, _accountType: ImageAccountType): ImageModel {
  return "gpt-image-2";
}

function normalizeStoredGenerationConfig(value: unknown): StoredImageGenerationConfig | undefined {
  if (!value || typeof value !== "object") {
    return undefined;
  }

  const config = value as Record<string, unknown>;
  const accountType = normalizeStoredAccountType(config.accountType || config.account_type);
  const aspectRatio = normalizeStoredAspectRatio(config.aspectRatio, config.size);
  const resolution =
    accountType === "free" ? "1k" : normalizeImageResolutionForAspectRatio(config.resolution, aspectRatio);
  return {
    model: normalizeStoredModel(config.model, accountType),
    accountType,
    count: Math.max(1, Math.floor(Number(config.count || 1) || 1)),
    size: typeof config.size === "string" ? config.size : "",
    aspectRatio,
    resolution,
    quality: accountType === "free" ? "auto" : normalizeStoredQuality(config.quality),
  };
}
...
function normalizeTurn(turn: ImageTurn & Record<string, unknown>): ImageTurn {
  ...
  const accountType = normalizeStoredAccountType(turn.accountType || turn.account_type);
  return {
    id: String(turn.id || `${Date.now()}`),
    prompt: String(turn.prompt || ""),
    model: normalizeStoredModel(turn.model, accountType),
    accountType,
    mode: turn.mode === "edit" ? "edit" : "generate",
    referenceImages: getLegacyReferenceImages(turn),
    count: Math.max(1, Number(turn.count || normalizedImages.length || 1)),
    size: typeof turn.size === "string" ? turn.size : "",
    aspectRatio: normalizeStoredAspectRatio(turn.aspectRatio, turn.size),
    resolution: normalizeStoredResolution(turn.resolution),
    quality: normalizeStoredQuality(turn.quality),
...
function normalizeConversation(conversation: ImageConversation & Record<string, unknown>): ImageConversation {
  const legacyAccountType = normalizeStoredAccountType(conversation.accountType || conversation.account_type);
  const turns = Array.isArray(conversation.turns)
    ...
    : [
        normalizeTurn({
          ...
          model: normalizeStoredModel(conversation.model, legacyAccountType),
          ...
          aspectRatio: normalizeStoredAspectRatio(conversation.aspectRatio, conversation.size),
          resolution: normalizeStoredResolution(conversation.resolution),
          quality: normalizeStoredQuality(conversation.quality),
