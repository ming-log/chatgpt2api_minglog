...
import { formatImageOutputSize, type ImageQuality } from "@/lib/image-generation-options";
...
type ImageResultsProps = {
  ...
  onContinueEdit: (conversationId: string, image: StoredImage | StoredReferenceImage, sourceTurnId?: string) => void;
...
export function ImageResults({
  ...
  const hasActiveImageTask =
    selectedConversation?.turns.some(
      (turn) =>
        !turn.resultsDeleted &&
            ...
            ? [
                {
                  id: image.id,
                  src,
                  thumbnailSrc: src.startsWith("data:") ? src : getImageThumbnailUrl(src),
              ...
              <div className="flex justify-start">
                <div className="w-full p-1">
                  ...
                  <div className="mb-3 flex flex-wrap items-center gap-1.5 text-[11px] text-stone-500 sm:mb-4 sm:gap-2 sm:text-xs">
                    ...
                    <span className="rounded-full bg-stone-100 px-3 py-1">{accountTypeLabel(turn.accountType)}</span>
                    <span className="rounded-full bg-stone-100 px-3 py-1">{turn.model}</span>
                    <span className="rounded-full bg-stone-100 px-3 py-1">{turn.aspectRatio}</span>
                    {turn.size ? <span className="rounded-full bg-stone-100 px-3 py-1">{formatImageOutputSize(turn.size)}</span> : null}
                    <span className="rounded-full bg-stone-100 px-3 py-1">{qualityLabel(turn.quality)}</span>
                  ...
                  <div className="grid grid-cols-3 gap-2 sm:block sm:columns-2 sm:gap-4 sm:space-y-4 xl:columns-3">
                    {turn.images.map((image, index) => {
                      ...
                      const elapsedLabel = formatElapsedTime(image.startedAt || turn.createdAt, nowMs);
                        ...
                        return (
                          ...
                          >
                            ...
                            >
                              <ImageThumbnail
                                src={imageSrc}
                                thumbnailSrc={imageSrc.startsWith("data:") ? imageSrc : getImageThumbnailUrl(imageSrc)}
                            ...
                            <div className="flex flex-col gap-1 px-0.5 py-1 text-[10px] sm:flex-row sm:items-center sm:justify-between sm:gap-2 sm:px-3 sm:py-3 sm:text-xs">
                              ...
                              <div className="flex items-center gap-1.5">
                                <Button
                                  ...
                                  onClick={() => onContinueEdit(selectedConversation.id, image, turn.id)}
                                  aria-label="加入编辑"
                                >
                        ...
                        return (
                          <div
                            ...
                            className={cn(
                              ...
                              "aspect-square",
                              turn.aspectRatio === "1:1" && "sm:aspect-square",
                              turn.aspectRatio === "16:9" && "sm:aspect-video",
                              turn.aspectRatio === "9:16" && "sm:aspect-[9/16]",
                              turn.aspectRatio === "4:3" && "sm:aspect-[4/3]",
                              turn.aspectRatio === "3:4" && "sm:aspect-[3/4]",
                            )}
                          >
                      ...
                      return (
                        <div
                          ...
                          className={cn(
                            ...
                            turn.aspectRatio === "1:1" && "aspect-square",
                            turn.aspectRatio === "16:9" && "aspect-video",
                            turn.aspectRatio === "9:16" && "aspect-[9/16]",
                            turn.aspectRatio === "4:3" && "aspect-[4/3]",
                            turn.aspectRatio === "3:4" && "aspect-[3/4]",
                          )}
                        >
                          <div className="flex h-full flex-col items-center justify-center gap-1.5 px-2 py-3 text-center text-stone-500 sm:gap-3 sm:px-6 sm:py-8">
                            ...
                            <div className="flex flex-col gap-0.5">
                              ...
                              {elapsedLabel ? (
                                <p className="text-[10px] leading-4 text-stone-400 sm:text-xs">耗时 {elapsedLabel}</p>
                              ) : null}
                            </div>
                          </div>
                        </div>
