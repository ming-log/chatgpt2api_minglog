...
function isPaidAccountType(value: unknown) {
...
function isUnlimitedImageQuotaAccount(account: Account) {
  const type = normalizeAccountTypeValue(account.type);
...
function displayAccountType(account: Account) {
  return isPaidAccountType(account.type) ? "Team/Plus/Pro" : "Free";
}

const TYPE_BADGE_CLASS: Record<string, string> = {
  Free: "border-stone-200 bg-stone-50 text-stone-700",
  "Team/Plus/Pro": "border-blue-200 bg-blue-50 text-blue-700",
};
...
function quotaUpperBound(account: Account) {
  ...
  // 兜底：按免费 / 付费类型估算，主要用于历史数据还没来得及补 initial_quota 的场景。
  const initial = Math.max(0, Math.floor(account.initial_quota ?? 0));
  if (initial > 0) return initial;
  return isPaidAccountType(account.type) ? 50 : 25;
}
...
function AccountsPageContent() {
  ...
  useEffect(() => {
    const isActive = isLoading || isRefreshing || isDeleting || isExporting;
    if (isActive) {
  ...
  }, [isLoading, isRefreshing, isDeleting, isExporting]);
    ...
    <>
      ...
      <section className="mt-4 flex flex-col gap-4 sm:mt-6 lg:flex-row lg:items-end lg:justify-between">
        ...
        <div className="flex flex-wrap items-center gap-2">
          ...
          <AccountImportDialog
            disabled={showProgress || isExporting}
            onImported={(items) => {
              setAccounts(items);
              setSelectedIds([]);
              setPage(1);
