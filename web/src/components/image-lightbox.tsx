...
type ImageLightboxProps = {
  ...
  onDownload?: (image: LightboxImage) => void | Promise<void>;
  onImageLoad?: (image: LightboxImage, metadata: { width: number; height: number }) => void;
};
...
function filenameFromImage(image: LightboxImage) {
  if (image.downloadName) return image.downloadName;
  try {
    if (image.src.startsWith("data:")) {
      const mime = image.src.slice(5, image.src.indexOf(";"));
      return `image-${image.id}.${extensionFromMime(mime)}`;
    }
...
function triggerDownload(href: string, filename: string, targetBlank = false) {
...
export function ImageLightbox({
  ...
  const metadataLabel = current?.metadataLabel || [current?.sizeLabel, current?.dimensions].filter(Boolean).join(" · ");
  ...
  const handleDownload = useCallback(async () => {
    ...
    try {
      ...
      const filename = filenameFromImage(current);
      if (current.src.startsWith("data:")) {
        triggerDownload(current.src, filename);
        return;
      }
      ...
      // Keep the user on the current tab even if the source URL cannot be fetched as a blob.
      triggerDownload(current.src, filenameFromImage(current), true);
    } finally {
    ...
    <DialogPrimitive.Root open={open} onOpenChange={onOpenChange}>
      <DialogPrimitive.Portal>
        ...
        >
          ...
          <div className="absolute top-[calc(env(safe-area-inset-top)+1rem)] right-4 left-4 z-10 flex items-start justify-end gap-2">
            {metadataLabel ? (
              <span className="max-w-[calc(100vw-8rem)] rounded-full bg-black/50 px-3 py-1.5 text-right text-xs font-medium leading-5 text-white/90 sm:max-w-[min(68vw,900px)]">
                {metadataLabel}
              </span>
            ) : null}
            {images.length > 1 && (
